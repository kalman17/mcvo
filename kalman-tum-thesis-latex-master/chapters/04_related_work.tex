...
Since the proposed framework (Chapter~\ref{chapter:solution-approach}) builds directly on AnyCam as the pose estimator, its architecture is illustrated in detail in Figure~\ref{fig:anycam-pipeline}.
...
Rather than building a new model from scratch, our approach proposes a framework for \emph{fusing} existing pretrained models: a dedicated calibration network provides accurate intrinsics, a multi-frame aggregation module (the MCT) enforces temporal consistency, and a self-supervised pose estimator leverages both. The resulting system satisfies all key requirements in Table~\ref{tab:related-work-comparison}: label-free training, feed-forward inference, multi-frame calibration consistency, dynamic scene robustness, and model-agnostic calibration. We demonstrate this framework by fusing AnyCam and AnyCalib, though the architecture is not specific to these models.