...
\section{FAT Calibration Head Training Progression}\label{sec:app-fat-progression}

FAT calibration head training proceeded through two stages on Objectron:

\textbf{Stage B1} (reprojection loss, no visual tokens) converged successfully with stable training. Validation loss decreased steadily over 50 epochs, and per-epoch benchmarking showed consistent improvements in calibration accuracy. The final MAPE was competitive with AnyCalib's per-frame average, demonstrating that FAT can aggregate spatial features effectively.

An intermediate attempt to add global visual context via DINOv2-small CLS tokens resulted in immediate overfitting on the limited Objectron dataset. Validation loss increased from 8.61 to 14.37 to 19.10 across the first three epochs while training loss continued to decrease. This classic overfitting pattern indicates that visual tokens add model capacity but the Objectron dataset is too small and homogeneous to support learning generalizable visual-calibration relationships. This variant was abandoned.

\textbf{Stage B2} (integration with pose pipeline) connects FAT to the full AnyCam pipeline using a combined loss of flow reprojection and calibration anchor terms. Training on Objectron was initiated, with full results expected from cluster training on diverse datasets where the larger and more varied training distribution should prevent overfitting.
