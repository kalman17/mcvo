#!/bin/bash
#SBATCH --job-name=Cb3s_48g
#SBATCH --output=/storage/user/maka/logs/train_Cb_v3s_48g_%j.out
#SBATCH --error=/storage/user/maka/logs/train_Cb_v3s_48g_%j.err
...
TRAIN_DIR="/storage/user/maka/train/phase_Cb_v3s_48g"

# Catch up to H100's best epoch checkpoint, or own latest on preemption
INIT_CKPT="/storage/user/maka/train/phase_Cb_v3s_h100/checkpoints/epoch_0004.pt"
...
echo "============================================"
echo "  Phase Cb v3s 48GB — v3 continued with stability fixes"
...
if [ -f "$TRAIN_DIR/checkpoints/latest.pt" ]; then
    ...
    echo "Starting from H100 epoch 4 checkpoint (weights only)"
    RESUME_ARGS="--resume $INIT_CKPT --resume_weights_only"
fi

echo ""
echo "=== Phase Cb v3s 48GB Training (20 epochs, batch_size=6, lr=2e-5 cosine, wd=1e-5, clip=0.3, no comp) ==="
...
echo ""
echo "=== Phase Cb v3s 48GB COMPLETE ==="
echo "Date: $(date)"
