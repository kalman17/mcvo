#!/bin/bash
#SBATCH --job-name=Cb6_h100
#SBATCH --output=/storage/user/maka/logs/train_Cb_v6_h100_%j.out
#SBATCH --error=/storage/user/maka/logs/train_Cb_v6_h100_%j.err
...
TRAIN_DIR="/storage/user/maka/train/phase_Cb_v6_h100"
...
echo "============================================"
echo "  Phase Cb v6 H100 — Proper flow composition"
...
if [ -f "$TRAIN_DIR/checkpoints/latest.pt" ]; then
    ...
    echo "Starting from v3 epoch 5 checkpoint (weights only)"
    RESUME_ARGS="--resume $INIT_CKPT --resume_weights_only"
fi

echo ""
echo "=== Phase Cb v6 H100 Training (20 epochs, batch_size=10, lr=2e-5 cosine, wd=1e-5, clip=0.3, comp=0.5) ==="
...
echo ""
echo "=== Phase Cb v6 H100 COMPLETE ==="
echo "Date: $(date)"
